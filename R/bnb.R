...
brandAndBound <- function(Y, K = c(3), bs = 200, aggType = 1, alpha = .05, pValueAgg = "tippett", intercept = 1, verbose = T){
  ...
  .testAncest <- function(ancest){
    ...
    pVals <- cdcs::bnbHelperMulti(ancest - 1, possibleChildren - 1, Y, K = K, aggType = aggType, bs = bs, intercept = intercept)
  ...
  while(dim(currentSeq)[1] > 0 & dim(currentSeq)[2] <= p){
    ...
    if(verbose){
      ...
      print(paste("Number of Perm: ", dim(currentSeq)[1] , sep = ""))
