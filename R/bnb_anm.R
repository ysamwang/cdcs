...
#' }
#' @param alpha the level of the confidence set
...
brandAndBound_anm <- function(Y, G, bs = 200,
  ...
  ancestMat <- matrix(0, n, p * K)
  if(basis == "poly"){
    
    for(i in 1:p){
      ancestMat[, (K * (i-1) + 1):(K*i)] <- cdcs::getHermite(Y[, i], K)
    }
    
  } else if (basis == "bspline"){
    
    for(i in 1:p){
      ancestMat[, (K * (i-1) + 1):(K*i)] <- splines::bs(Y[, i], df = K)
    }
    
  } else {
    stop("Incorrect basis type: must be either 'poly', 'bspline'")
  }
  ...
  .updatePvals <- function(nativeInd){
    ...
    if(sum(continueOn) > 0){
      ...
      # Reamining columns: Orderings which have not passed the cut-off
  ...
  .testAncest <- function(ancest){
    ...
    # cat(cdcs::mapVarToInd(ancest, K))
    pVals <- cdcs::bnbHelperanm(ancestMat[, cdcs::mapVarToInd(ancest, K), drop = F], Y[, possibleChildren, drop = F],
                                G[, , ancest, drop = F], withinAgg = aggType, aggType = aggType, 
