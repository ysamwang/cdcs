...
/*
 * Helper for R function 'brandAndBound'
  ...
  for(int j = 0; j < children.n_elem; j++){
    ...
    for(int k_ind = 0; k_ind < k_len; k_ind++){
      ...
      if ( aggType == 0){
        ...
        testStat += max( pow(X, K(k_ind)).t() * res) / sqrt(n);
        
      } else if (aggType == 1){
        
        testStat += accu( abs(pow(X, K(k_ind)).t() * res) ) / sqrt(n) ;
        
      } else {
        
        testStat += norm( pow(X, K(k_ind)).t() * res / sqrt(n), "fro") ;
