...
run.once <- function(p, n, distro, bs = 200, parent_prob = 1/2, verbose = F,
  ...
  # Updated gFunc to regress first
  gFunc <- function(x, k){
    if(k == 1){
      return(scale(RcppArmadillo::fastLm(y = x^3, X = cbind(rep(1, length(x)),x))$res))
    } else if (k ==2 ){
      return(scale(RcppArmadillo::fastLm(y = x^3, X = cbind(rep(1, length(x)),x))$res))
    } else if (k ==3 ){
      return( scale(RcppArmadillo::fastLm(y = abs(x)^(2.5) * sign(x), X = cbind(rep(1, length(x)),x))$res))
    }
  ...
  rec[3] <- cdcs::bnbHelperanm1(parents, child, G = G, withinAgg = 2, aggType = 1, bs = bs, intercept = 1)$pVals[1]
  rec[4] <- cdcs::bnbHelperanm1(parents, child, G = G, withinAgg = 2, aggType = 2, bs = bs, intercept = 1)$pVals[1]
  # rec[5] <- cdcs::senSen2014(parents, child, bs = bs)
  # rec[6] <- RPtests::RPtest(parents, child, B = bs, resid_type = "OLS")
  ...
  rec[2 * offset + 3] <- microbenchmark::microbenchmark(rec[offset + 3] <- cdcs::bnbHelperanm1(parents, child, G = G, withinAgg = 2,
                                                                                               aggType = 1, bs = bs, intercept = 1)$pVals[1], times = 1)$time
  rec[2 * offset + 4] <- microbenchmark::microbenchmark(rec[offset + 4] <- cdcs::bnbHelperanm1(parents, child, G = G, withinAgg = 2,
                                                                                               aggType = 2, bs = bs, intercept = 1)$pVals[1], times = 1)$time
...
out <- t(parSapply(cl, 1:rep.runs, function(x){run.once(p, n, distro, parent_prob = parent_prob, bs = 250)}))

# out <- t(parSapply(cl, 1:3, function(x){run.once(p, n, distro, parent_prob = parent_prob, bs = 250)}))



outTab <- data.frame(p, n, distro, parent_prob, pow.i, out)
...
write.csv(outTab, paste("../results/lmgdDAGNew/lmgdDAG_alt2_", runInd, ".csv", sep = ""), row.names = F)
...
#
runInd <- 1
outTab <- read.csv(paste("~/Dropbox/confSetGraphs/code/rPkg/simResults/lmgdDAGNew/lmgdDAG_alt2_", runInd, ".csv", sep = ""))
...
for(runInd in 2:nrow(param.grid)){
  temp <- read.csv(paste("~/Dropbox/confSetGraphs/code/rPkg/simResults/lmgdDAGNew/lmgdDAG_alt2_", runInd, ".csv", sep = ""))
